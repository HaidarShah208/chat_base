...
const CreateNewChatbot = () => {
  ...
  const handleCreateAgent = () => {
    ...
    navigate(`/dashboard/chatbot/${randomId}`);
  }

  return (
    <div className="w-full container md:p-0 p-4 mx-auto md:px-8 lg:px-12">
      <h1 className="text-3xl font-bold mb-6 mt-4 md:mt-8">Create new agent</h1>
      <div className="flex flex-col lg:flex-row w-full min-h-[80vh] gap-6 max-w-[1400px] mx-auto">
        <Sidebar
          tabs={sidebarTabs}
          activeTab={activeTab}
          onTabClick={(route) => navigate(`/dashboard/create-new-chatbot/${route}`)}
        />
        ...
        <div className="w-full lg:w-80 bg-white rounded-xl border p-6 shadow-sm h-fit flex flex-col gap-4">
          ...
          <button
            className={`font-semibold py-2 rounded-lg text-sm mt-4 transition ${
              totalSize > maxSize
